...
const AdminDashboard = {
  ...
  computed: {
    stats() {
      ...
      const codes     = Store.getInviteCodes();
...
const AdminTeachers = {
  ...
  computed: {
    teachers() {
      ...
      return Store.getTeachers().filter(t =>
  ...
  methods: {
    ...
    doReset() {
      ...
      Store.resetTeacherPassword(this.resetTarget.id, this.newPassword);
    ...
    doDelete() {
      const result = Store.deleteTeacher(this.confirmDeleteId);
      if (result.success) {
...
const AdminInviteCodes = {
  ...
  methods: {
    doAdd() {
      ...
      const result = Store.addInviteCode(this.newCode, this.newNote);
      if (result.success) {
...
const AdminStudents = {
  ...
  methods: {
    ...
    doReset() {
      ...
      Store.resetStudentPassword(this.resetTarget.id, this.newPassword);
      Store.toast(`✅ 已重置 ${this.resetTarget.name} 的密码`, 'success');
      this.resetModal = false;
    },
  },
...
const AdminSettings = {
  ...
  computed: {
    adminUsername() {
      // 优先读 localStorage 中被修改过的账号
      const saved = localStorage.getItem('pet_admin_override');
      if (saved) {
        try { return JSON.parse(saved).username || ADMIN_ACCOUNT.username; } catch(e) {}
      }
  ...
  methods: {
    ...
    doEditAdmin() {
      ...
      // 验证旧密码
      const saved = localStorage.getItem('pet_admin_override');
      const current = saved ? JSON.parse(saved) : { username: ADMIN_ACCOUNT.username, password: ADMIN_ACCOUNT.password };
      if (this.editOldPassword !== current.password) {
      ...
      const newData = {
        ...
        password: this.editNewPassword || current.password,
      };
      // 同步到 ADMIN_ACCOUNT（运行时生效）
      ADMIN_ACCOUNT.username = newData.username;
      ADMIN_ACCOUNT.password = newData.password;
      // 持久化
      localStorage.setItem('pet_admin_override', JSON.stringify(newData));
      Store.toast('✅ 管理员账号信息已更新', 'success');
      this.showEditAdmin = false;
    },
  },
  mounted() {
    // 启动时从 localStorage 恢复覆盖
    const saved = localStorage.getItem('pet_admin_override');
    if (saved) {
      try {
